...
shellcmd xsh_ps(int nargs, char *args[])
{
	...
	printf("%3s %-16s %5s %-8s %4s %10s %-10s %10s %-10s\n",
		   "Pid", "Name", "State", "Prio", "Ppid", "Stack Base",
		   "Stack Ptr", "Stack Size", "CPU Time");
	...
	for (i = 0; i < NPROC; i++) {
		...
		printf("%3d %-16s %s %-6u %4d 0x%08X 0x%08X %10d %10u\n",
			i, prptr->prname, pstate[(int)prptr->prstate],
			prptr->prprio, prptr->prparent, prptr->prstkbase,
			prptr->prstkptr, prptr->prstklen, prptr->prcpuused);
	}
	return 0;
}
